 ...
 *
 * Same-origin proxy for browser → gateway calls on the public shell origin.
 * Client Components fetch relative `/api/...` paths (so CORS never applies,
 * whatever port the composed dashboard runs on) and this handler forwards
 * them to the gateway using the server-side `API_URL` — read per request,
 * not baked into the bundle at build time the way `NEXT_PUBLIC_*` values
 * are. Remotes keep matching routes for direct-port development; the shell
 * is the route the browser actually hits when pages are composed via
 * multi-zone rewrites.
 */
