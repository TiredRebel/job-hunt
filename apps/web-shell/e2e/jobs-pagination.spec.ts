...
test('changing page size updates the URL and range readout', async ({ page }) => {
  ...
  const option50 = page.getByRole('option', { name: '50' }).first();
  await retryUntilHydrated(
    async () => {
      // Radix Select renders a scrollable listbox; when embedded in a
      // remote-composed shell it can be partially outside the viewport.
      await option50.scrollIntoViewIfNeeded();
      await option50.click();
    },
    () => expect(page).toHaveURL(/[?&]limit=50/),
  );
