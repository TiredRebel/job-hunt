...
test('sorts jobs without duplicating the locale in the URL', async ({ page }) => {
  ...
  await retryUntilHydrated(
    () => scoreHeader.click(),
    () => expect(page).toHaveURL(/\/uk\/jobs\?(?:[^#]*&)?sortBy=score(?:&|$)/),
  );
  await expect(page).toHaveURL(/sortDir=desc/);
  // Locale must appear once as the path prefix, not duplicated by the sort link.
  expect(new URL(page.url()).pathname).toBe('/uk/jobs');
  await expect(page.locator('main')).toBeVisible();
});
