 ...
 * @module env
 *
 * Environment contract for the dashboard API base URLs. Server-side code
 * talks to the gateway directly via `API_URL`; the browser defaults to the
 * same-origin `/api` proxy, which forwards to `API_URL` at request time —
 * so no gateway address is baked into the browser bundle and CORS never
 * applies. `NEXT_PUBLIC_API_URL` remains an optional override for pointing
 * the browser straight at the gateway (build-time inlined, requires the
 * gateway's `WEB_ORIGIN` to allow the web app's origin).
 */
import { z } from 'zod';
