...
The `/sources` page SHALL list all sources from `GET /v1/sources` with: name/slug, an enable/disable switch calling `PATCH /v1/sources/{slug}/enabled`, the schedule (cron expression with a human-readable hint), the last run's status and time, per-row Edit and Test actions, and — when the adapter registry is available — a "no adapter" indicator on sources whose slug has no registered scraper adapter.
...
- **THEN** every row exposes Edit and Test actions alongside the existing enable switch and "Run now" button
