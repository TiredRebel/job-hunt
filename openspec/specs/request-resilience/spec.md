...
Retry-with-backoff for transient cross-service HTTP failures — the
gateway's calls to the scraper/LLM services, and the LLM service's calls to
its provider adapters — applied only to calls that are safe to repeat.
...
after the final attempt the original error SHALL propagate unchanged. When a
429 response carries a `Retry-After` header, the gateway's retry client
SHALL honor it as the backoff delay in preference to the computed
exponential delay, but SHALL cap the resulting wait (whether server-directed
or computed) at a fixed maximum, so a malicious or misconfigured downstream
cannot stall a caller for an unbounded duration by returning an
arbitrarily large `Retry-After` value. Non-transient failures (4xx other
than 429) SHALL NOT be retried. Requests that are not safe to repeat SHALL
...
#### Scenario: A large Retry-After is capped

- **WHEN** a downstream call returns 429 with a `Retry-After` header
  requesting a delay longer than the configured maximum
- **THEN** the caller waits only the capped maximum, not the full requested
  duration
