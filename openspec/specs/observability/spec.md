...
Structured JSON logging in every service and end-to-end `X-Correlation-Id`
propagation across web → gateway → scraper/LLM, so a single request can be
...
absent or malformed, mint a new one (a UUID). A well-formed id is a bounded-
length string restricted to characters that cannot inject a forged log
field or header value when echoed back or embedded in JSON logs; a
caller-supplied value outside that format is discarded rather than trusted
verbatim. The service SHALL bind that id to every log line produced while
handling the request, echo it back on the response as `X-Correlation-Id`,
and, when it calls another service in the course of handling the request,
forward the same id on the outbound `X-Correlation-Id` header. Well-formed
ids received from callers SHALL be preferred over newly minted ones so the
same value spans the whole chain.
...
#### Scenario: A malformed id is rejected rather than trusted

- **WHEN** a request arrives carrying an `X-Correlation-Id` value that is
  empty, exceeds the maximum length, or contains characters outside the
  allowed safe format
- **THEN** the service discards it and mints a fresh id instead of logging
  or echoing the caller-supplied value verbatim
