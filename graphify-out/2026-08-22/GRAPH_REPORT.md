# Graph Report - job-hunter (2026-08-22)

## Corpus Check

- 630 files · ~341,036 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary

- 5461 nodes · 9522 edges · 384 communities (334 shown, 50 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 488 edges (avg confidence: 0.59)
- Token cost: 0 input · 0 output

## Graph Freshness

- Built from commit: `b8bff77c`
...
- `JobTable()` --calls--> `buildJobColumns()` [EXTRACTED]
  apps/web/src/components/jobs/job-table.tsx → apps/web/src/components/jobs/job-table-columns.tsx
...
- `DropdownMenuSeparator()` --calls--> `cn()` [EXTRACTED]
  apps/web/src/components/ui/dropdown-menu.tsx → apps/web/src/lib/utils.ts
- `match()` --uses--> `MatchRequest` [INFERRED]
  services/llm/src/llm/routes.py → services/llm/src/llm/api.py
...
Cohesion: 0.10
Nodes (21): dependencies, class-transformer, nestjs-cls, @nestjs/common, @nestjs/config, @nestjs/core, @nestjs/swagger, pino (+13 more)
...
Cohesion: 0.10
Nodes (53): FakeDb, fake_provider(), FakeProvider, fixture, In-memory `LLMProvider` returning canned responses per schema type., Tests for the REST surface (fakes injected via app state, no real I/O)., A row activated between the active-check and the delete itself still 409s., test_cover_letter_endpoint() (+45 more)

### Community 19 - "scraper/main.py"

Cohesion: 0.06
Nodes (45): ge, le, get_settings(), BaseSettings, Runtime configuration for the scraper service. Settings come from environment…, Scraper service settings. Attributes: database_url: PostgreSQL DSN…, Return the process-wide settings singleton. Returns: Cached :class:`Settings`…, Settings (+37 more)
...
### Community 21 - "SearchQuery"

Cohesion: 0.07
Nodes (32): _matches(), parse_listing(), Any, Reddit adapter — public JSON API, no scraping (docs/SOURCES.md). Reads…, Materialize a posting from the cached listing entry (no re-fetch). Args: lead:…, Fetch (or reuse) the `new` listing for one subreddit. Args: subreddit:…, Extract post objects from a Reddit listing payload. Args: payload: Decoded…, Check whether a post satisfies the flair and term filters. Args: post: Reddit… (+24 more)
...
Cohesion: 0.13
Nodes (16): cover_letter_system(), normalize_prompt(), Pick the cover-letter system prompt formatted for the provider's family. Claude…, Build the user prompt for the `normalize` pipeline., Build the user prompt for the `tag` pipeline., tag_prompt(), Tests for provider-aware cover-letter system prompt selection., test_anthropic_gets_xml_structured_prompt() (+8 more)

### Community 32 - "run_scrape"

Cohesion: 0.11
Nodes (24): Lifecycle states of a scrape run (mirrors `scraper.scrape_runs.status`)., RunStatus, Outcome of a completed scrape run. Attributes: run_id: Id of the…, Execute one scrape run for `source` using `adapter`. Per-lead failures are…, run_scrape(), RunResult, Record that the run was finalized., FakeAdapter (+16 more)
...
### Community 41 - "filter-bar.tsx"

Cohesion: 0.09
Nodes (31): BulkActionBar(), BulkActionBarProps, STAGE_OPTIONS, DATE_PRESETS, dateInputValue(), FilterBar(), FilterChip, parseDateInput() (+23 more)

### Community 42 - "CompletionRequest"

Cohesion: 0.03
Nodes (86): AsyncRetrying, BaseException, RetryCallState, MissingApiKeyError, ProviderRequestError, The upstream provider HTTP call failed., The provider reply did not validate against the pipeline schema., The provider row names an env var that is not set. (+78 more)

### Community 43 - "Database"

Cohesion: 0.06
Nodes (24): Database, DeadLetterRow, TypedDict, Load enabled search dictionaries (re-read on every run). Returns: Rows feeding…, Load enabled filter dictionaries (re-read on every run). Returns: Rows feeding…, Insert a `running` scrape-run row. Args: source_id: FK into `core.sources`.…, Finalize a scrape-run row. Args: run_id: Id returned by :meth:`create_run`.…, Persist a raw posting, deduplicating on the unique constraint. Duplicate rows… (+16 more)
...
### Community 45 - "_run_provider_test"

Cohesion: 0.10
Nodes (31): BuildProviderDep, CredentialCipherDep, DbDep, InternalTokenDep, ProviderTestResponse, ResolverDep, ProviderPublic, ProviderTestResponse (+23 more)

### Community 46 - "JobLead"

Cohesion: 0.06
Nodes (59): build_posting(), extract_text(), Shared mechanics and helpers for static-HTML source adapters. Source-specific…, Extract normalized text from the first node matching `selector`. Falls back…, Assemble a :class:`RawJobPosting` with a content-based fingerprint.…, Immutable source-specific configuration for a static HTML adapter. Attributes:…, Implement the shared discovery and detail lifecycle for static HTML. The…, Initialize a static HTML adapter. Args: source: Immutable source mechanics and… (+51 more)

### Community 47 - "llm.ts"

Cohesion: 0.10
Nodes (29): dynamic, LlmSettingsPageClient(), ConnectionTestState, kindBucket(), ProviderCard(), ProviderCardProps, buildOverridesPayload(), parseTemperature() (+21 more)

### Community 48 - "cn"

Cohesion: 0.07
Nodes (45): MultiSelectOption, MultiSelectProps, SHORTCUT_GROUPS, ShortcutsDialogProps, DraftTestState, FormState, initialState(), ModelCombobox() (+37 more)
...
Cohesion: 0.05
Nodes (55): dynamic, JobsPage(), JobsPageProps, { refreshMock }, FilterBarProps, JobsClient(), JobsClientProps, replace (+47 more)

### Community 54 - "automation.service.ts"

Cohesion: 0.08
Nodes (23): AUTOMATION_REPOSITORY, AutomationRepository, CoverLetterInput, DigestJobSummary, DigestMatchSummary, DigestPayload, MatchInput, NormalizedJobInput (+15 more)
...
### Community 56 - "reconciliation.service.spec.ts"

Cohesion: 0.17
Nodes (12): JOBS_RECONCILIATION_REPOSITORY, JobsReconciliationRepository, ReconciliationAggregate, ReconciliationRow, mapRow(), PostgresJobsReconciliationRepository, ReconciliationDbRow, Injectable (+4 more)

### Community 57 - "sources-page.tsx"

Cohesion: 0.06
Nodes (61): dynamic, CommandPalette(), useDebouncedValue(), IN_MOTION_REACTIONS, LatestSourceRun, Sidebar(), SidebarProps, useNavData() (+53 more)
...
Cohesion: 0.13
Nodes (26): BulkDeletedResponse, BulkInsertedResponse, DeletedResponse, ApiProperty, PaginatedResponse(), PaginatedShape, RemoteType, Seniority (+18 more)
...
Cohesion: 0.10
Nodes (37): Persistence layer: PostgreSQL access via psycopg (async pool). Only this module…, Row of `core.sources` used by the runner., SourceRow, _applies_to(), build_filter_rules(), company_matches(), FilterDictionaryRow, FilterRules (+29 more)

### Community 62 - "Profile"

Cohesion: 0.08
Nodes (19): CreateProfileInput, PROFILE_REPOSITORY, ProfileRepository, UpdateProfileInput, FakeProfileRepository, FakeProfileRepository, CvLanguage, CvMarkdownByLanguage (+11 more)
...
Cohesion: 0.10
Nodes (22): ApiConflictResponse, SourcesController, ApiBody, ApiCreatedResponse, ApiNotFoundResponse, ApiOkResponse, ApiOperation, ApiParam (+14 more)
...
Cohesion: 0.06
Nodes (44): FetchActive, ProviderRow, Any, BaseModel, PostgreSQL access for the LLM service (registry reads, run bookkeeping). Tables…, Return one registry row by slug, or `None` if unknown., Update only the provided fields and broadcast `NOTIFY`.…, Activate `slug` and broadcast `NOTIFY llm_config_changed`. Raises:… (+36 more)
...
### Community 71 - "profile-form.tsx"

Cohesion: 0.08
Nodes (28): dynamic, ProfilePage(), BreakdownRow, buildRows(), MatchExplanationBars(), MatchExplanationBarsProps, salaryMidpoint(), translate (+20 more)
...
### Community 74 - "automation.module.ts"

Cohesion: 0.06
Nodes (22): DeadLetterJob, RawJob, RawJobOutcome, SCRAPER_CLIENT, ScraperClient, ScrapeTriggerResponse, SourceTestResult, FakeScraperClient (+14 more)
...
Cohesion: 0.04
Nodes (39): PLACEHOLDER_ENV, AppModule, Module, AutomationModule, Module, BigIntSerializerInterceptor, serializeBigInts(), Injectable (+31 more)
...
Cohesion: 0.06
Nodes (41): asyncio, RuntimeError, Fetch the configured listing URL for a connectivity check. Returns: The…, Fetch the first configured subreddit's listing, for connectivity testing., Fetch the RSS feed once, for connectivity testing. Unlike :meth:`discover`, a…, _extract_text(), agent-browser subprocess fetcher for JS-heavy, non-API sources. Only Upwork is…, Invoke the CLI and map its outcome to fetcher semantics. Isolated as its own… (+33 more)
...
Cohesion: 0.19
Nodes (13): buildPatch(), fromSettings(), NotificationFormState, NotificationSettingsForm(), validate(), ValidationErrors, Switch(), SwitchProps (+5 more)
...
### Community 130 - "job-drawer.tsx"

Cohesion: 0.23
Nodes (9): JobDrawer(), replace, sheetState, JobDrawer, Sheet(), SheetContent(), SheetDescription(), SheetTitle() (+1 more)
...
Cohesion: 0.10
Nodes (30): BackgroundTasks, LookupError, post, Schedule a scrape run for one source. The run row is created synchronously so…, Test connectivity for one source without persisting anything. Resolves the…, test_source(), trigger_scrape(), create_adapter() (+22 more)
...
### Community 138 - "Crawl4aiFetcher"

Cohesion: 0.15
Nodes (18): AsyncWebCrawler, Crawl4aiFetcher, Render `url` and map the result/failure to fetcher semantics. Isolated as its…, Browser-rendered fetcher via crawl4ai's `AsyncWebCrawler`. The browser is…, Render `url` politely through crawl4ai. Args: url: Absolute URL to fetch.…, Close the underlying browser, if one was started., Lazily start (once) and return the underlying crawler. Raises:…, _gate() (+10 more)
...
Cohesion: 0.07
Nodes (30): DateField, JOB_REPOSITORY, JobFilter, JobRepository, JobSortBy, PaginatedJobs, SortDir, FakeJobRepository (+22 more)
...
Cohesion: 0.07
Nodes (26): nextConfig, withNextIntl, geistSans, jetbrainsMono, metadata, createQueryClient(), QueryProvider(), ThemeProvider() (+18 more)
...
Cohesion: 0.08
Nodes (9): PgDatabase, buildOrderBy(), mapJobRow(), PostgresJobRepository, CaptureDatabase, COUNT_METRICS, JobRow, QueryCall (+1 more)
...
### Community 227 - "routes.py"

Cohesion: 0.04
Nodes (68): AsyncConnectionPool, ModelListResponse, ModelListResponse, Outcome of `GET /providers/{slug}/models`., get_settings(), BaseSettings, Runtime configuration for the LLM service. Settings come from environment…, LLM service settings read from the environment. (+60 more)
...
### Community 230 - "run_structured"

Cohesion: 0.10
Nodes (28): CoverLetter, GraphDepsDep, RunRecorder, PromptInjectionDetectedError, Store the missing environment-variable name., The composed prompt matched a known prompt-injection pattern., Store the matched signal labels (see `pipelines.injection_guard`)., ModelT (+20 more)
...
Cohesion: 0.09
Nodes (25): NOTIFICATION_SETTINGS_REPOSITORY, NotificationSettingsRepository, FakeNotificationSettingsRepository, FakeSourceRepository, NotificationSettings, UpdateNotificationSettingsInput, applyChannelPatch(), applyScalarPatch() (+17 more)
...
### Community 265 - ".persistResult"

Cohesion: 0.25
Nodes (6): ApiBody, ApiCreatedResponse, ApiParam, Body, Param, Post
...
Cohesion: 0.05
Nodes (42): dsMessages, DsPreviewProviderProps, DESIGN_OPTIONS, DesignMode, DesignModeToggle(), getClientSnapshot(), getServerSnapshot(), subscribeToHydration() (+34 more)
...
### Community 276 - "CorrelationIdMiddleware"
...
### Community 284 - "reconciliation.controller.ts"
...
Cohesion: 0.11
Nodes (19): AsyncBaseTransport, Initialize the fetcher. Args: gate: Shared politeness gate (robots + per-host…, Initialize the fetcher. Args: gate: Shared politeness gate (robots + per-host…, PolitenessGate, Check robots.txt for `url`, caching one parser per host., Per-host robots.txt cache and minimum-delay-with-jitter pacing. One instance is…, Initialize the gate. Args: user_agent: Descriptive UA string used both for…, Release the internal robots.txt HTTP client. (+11 more)
...
### Community 288 - "AutomationController"

Cohesion: 0.19
Nodes (10): ApiSecurity, AutomationController, ApiOkResponse, ApiOperation, ApiTags, Controller, Get, Query (+2 more)
...
### Community 295 - "AutomationService"

Cohesion: 0.36
Nodes (4): AutomationService, toLlmProfileInput(), toUnprocessedJob(), Injectable
...
### Community 317 - "internal-token.guard.ts"
...
Cohesion: 0.07
Nodes (64): CoverLetterRequest, CreateProviderRequest, MatchRequest, PipelineOverride, ProcessJobRequest, ProcessJobResponse, ProviderConnectionTestRequest, BaseModel (+56 more)

### Community 325 - "job-table.tsx"

Cohesion: 0.11
Nodes (24): DeadLetterPage(), dynamic, JobColumnsTranslations, JobRow, JobTable(), JobTableProps, SORTABLE_COLUMN_IDS, SORTABLE_COLUMN_TO_API (+16 more)
...
### Community 367 - "stage-board.tsx"

Cohesion: 0.05
Nodes (41): dynamic, BOARD_STAGES, BoardStage, ALL_JOBS, APPLIED_JOB, INTERVIEW_JOB, renderCountsByScore, SAVED_JOB (+33 more)

### Community 374 - "RawJobPosting"

Cohesion: 0.15
Nodes (11): Fetched vacancy payload ready for persistence into `scraper.jobs_raw`.…, RawJobPosting, Protocol, Ports (interfaces) of the scraper service. Adapters in :mod:`scraper.adapters`…, Port implemented by every job-source integration. Attributes: slug: Source…, Yield vacancy leads matching `query` from listing pages. Args: query: Search…, Fetch the detail payload for a discovered lead. Args: lead: Lead previously…, SourceAdapter (+3 more)

### Community 375 - "job-detail.tsx"

Cohesion: 0.08
Nodes (30): dynamic, JobDetailPageProps, CoverLetterEditor(), CoverLetterEditorProps, FOOTER_STAGES, footerStageLabel(), JobDetailView(), JobDetailViewProps (+22 more)
...
- **1723 isolated node(s):** `evaluatorDirectory`, `projectRoot`, `webRoot`, `nextBinary`, `port` (+1718 more)
...
- **Why does `build_fetcher_factory()` connect `scraper/main.py` to `fetchers/__init__.py`, `EscalatingFetcher`, `PageFetcher`, `ProviderRow`?**
  _High betweenness centrality (0.005) - this node is a cross-community bridge._
- **Why does `Database` connect `Database` to `run_scrape`, `scraper/main.py`, `SearchQuery`, `RawJobPosting`, `models.py`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
- **Why does `cn()` connect `cn` to `job-drawer.tsx`, `job-table.tsx`, `filter-bar.tsx`, `notification-settings-form.tsx`, `llm.ts`, `topbar.tsx`, `stage-board.tsx`, `dict-editor.tsx`, `job-detail.tsx`, `sources-page.tsx`?**
